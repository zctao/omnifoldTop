...
There are several dicts of keyword arguments setting global style for all plots
generated by this module (analogous to matplotlib's rcParams). `hist_style`,
`graph_style`, and `leg_style` set styles on histograms, graphs, and legends,
respectively. Where to look for documentation of the options:
...
The remainder (`data_style`, `sim_style`, `bkg_style`, `gen_style`, `truth_style`,
`ibu_style`, and `omnifold_style`) set styles for plotting from specific datasets (for
example, data is always plotted in orange with the label "Data").
"""
